...
#include "Cool/Window/internal/WindowFactory_Impl.h"
...
void WindowFactory::glfw_error_callback(int error, const char* description)
{
    Log::Debug::error_without_breakpoint("glfw", fmt::format("Error {}:\n{}", error, description));
}

Window& WindowFactory::make_main_window(const WindowConfig& config)
{
...
Window& WindowFactory::make_secondary_window(const WindowConfig& config)
{
    auto& window = _impl.make_window(config, _window_manager);
...
void WindowFactory::initialize_glfw()
{
    glfwSetErrorCallback(WindowFactory::glfw_error_callback);
    if (!glfwInit())
    {
        const char* error_description;
        glfwGetError(&error_description);
        Log::Debug::error("WindowFactory::initialize_glfw", fmt::format("Initialization failed:\n{}", error_description));
    }
}
...
void WindowFactory::initialize_imgui()
{
    // Setup context
    IMGUI_CHECKVERSION();
    ImGui::CreateContext();
