...
auto HSLuv_from_XYZ(glm::vec3 c) -> glm::vec3
{
    ...
    float l = (c.y <= Cool::epsilon) ? c.y * Cool::kappa :  116.f * std::cbrt(c.y) - 16.f ;
    ...
    if(l > 99.9999999f || l < 0.00000001f) s = 0.f;
    else s = c2 / Cool::max_chroma_for_lh(l, h)* 100.f;
...
auto XYZ_from_HSLuv(glm::vec3 c) -> glm::vec3{
    ...
    //lch -> LUV
    float hrad = h * 0.01745329251994329577f;  /* (pi / 180.0) */
    float u = cos(hrad) * c2;
    float v = sin(hrad) * c2;
