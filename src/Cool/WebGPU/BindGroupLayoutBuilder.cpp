#include "BindGroupLayoutBuilder.hpp"
...
auto BindGroupLayoutBuilder::read_texture_1D(int binding, std::optional<wgpu::ShaderStageFlags> visibility) -> BindGroupLayoutBuilder&
{
    BUILDER(
        _entries.back().texture.sampleType    = wgpu::TextureSampleType::UnfilterableFloat; // R32Float textures cannot be filtered
        _entries.back().texture.viewDimension = wgpu::TextureViewDimension::_1D;
    )
}
