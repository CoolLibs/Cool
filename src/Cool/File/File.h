...
public:
    ...
    static [[nodiscard("You can only use the folders if this function returns true")]] auto create_folders_if_they_dont_exist(std::filesystem::path folder_path) -> bool;

    /// Recursively creates all the folders so that at the end `file_path` is a valid file path.
    /// Returns true iff the folders now exist (be it because they already existed or because they have been successfully created).
    static [[nodiscard("You can only use the file if this function returns true")]] auto create_folders_for_file_if_they_dont_exist(std::filesystem::path file_path) -> bool;
