...
void Cool::test_did_you_know(Cool::DidYouKnow& did_you_know)
{
    if (ImGui::Button("Open a \"Did you know?\""))
        did_you_know.open_popup();

    // imgui text with next time of opening
    auto next_opening = (did_you_know._timestamp_last_opening + time_to_wait()).time_since_epoch();
    ...
    ImGui::TextUnformatted(fmt::format("Current timestamp: {:%OH:%OM:%OS}", did_you_know._timestamp_last_opening.time_since_epoch()).c_str());
    ...
    auto time_remaining = time_to_wait() - (std::chrono::steady_clock::now() - did_you_know._timestamp_last_opening);
