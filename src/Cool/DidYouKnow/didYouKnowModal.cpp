...
void debug_did_you_know(DidYouKnowModal& _did_you_know_)
{
    ...
    if (ImGui::Button("Test DidYouKnow"))
    {
        ...
        _did_you_know_._current_timestamp = std::chrono::system_clock::to_time_t(std::chrono::system_clock::now()) - (2 * 60 * 60);
    }
    ...
    ImGui::Text("Current timestamp: %s", std::to_string(_did_you_know_._current_timestamp).c_str());
