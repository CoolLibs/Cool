...
void Cool::test_tips(Cool::TipsManager& tips_manager)
{
    ...
    ImGui::TextUnformatted(fmt::format("Current timestamp: {:%OH:%OM:%OS}", tips_manager._timestamp_last_opening.time_since_epoch()).c_str());
    ...
    auto time_remaining = time_to_wait() - (std::chrono::steady_clock::now() - tips_manager._timestamp_last_opening);
