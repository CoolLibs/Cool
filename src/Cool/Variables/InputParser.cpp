...
) -> SharedVariableId<T>
{
    ...
    {
        input.desired_color_space = static_cast<int>(parse_color_space(type));
    }
    else if constexpr (std::is_same_v<T, Cool::ColorAndAlpha>)
    {
        input.desired_color_space = static_cast<int>(parse_color_and_alpha_space(type));
    }
...
{
    return COOL_TFS_EVALUATE_FUNCTION_TEMPLATE(make_input, type, AnySharedVariableId, (name, description, dirty_flag, input_factory, type));
}
...
{
    std::vector<AnySharedVariableId> new_inputs;
...
template<typename T>
auto gen_input_shader_code(const Variable<T>& input, Cool::ValueProvider input_provider) -> std::string
{
    return gen_input_shader_code<T>(input, input_provider, input.name());
}

template<typename T>
auto gen_input_shader_code(const Variable<T>& input, Cool::ValueProvider input_provider, std::string_view name /* Allows us to use a different name than the input's user-facing name if we want to */) -> std::string
{
    return gen_input_shader_code__impl(input_provider(input), name);
}

auto gen_input_shader_code(AnyVariable const& input, Cool::ValueProvider input_provider) -> std::string
{
    return std::visit([&](auto&& input) { return gen_input_shader_code(input, input_provider); }, input);
}

auto gen_input_shader_code(AnyVariable const& input, Cool::ValueProvider input_provider, std::string_view name) -> std::string
{
    return std::visit([&](auto&& input) { return gen_input_shader_code(input, input_provider, name); }, input);
}

static auto gen_input_shader_code(std::string_view name, const std::vector<AnyInput>& inputs, Cool::ValueProvider input_provider) -> std::string
{
    std::string res;
...
auto preprocess_inputs(std::string_view source_code, const std::vector<AnyInput>& inputs, Cool::ValueProvider input_provider) -> std::string
{
