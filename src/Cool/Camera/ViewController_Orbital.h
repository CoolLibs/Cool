...
public:
    ...
    inline void on_mouse_move(Camera& camera, glm::vec2 const& delta)
    {
        std::visit([&](auto&& state) { state.on_mouse_move(*this, camera, delta); }, _state);
    }
    inline void on_wheel_down(Camera& camera, int mods)
    {
        std::visit([&](auto&& state) { state.on_wheel_down(*this, camera, mods); }, _state);
    }
    inline void on_wheel_up(Camera& camera)
    {
        std::visit([&](auto&& state) { state.on_wheel_up(*this, camera); }, _state);
    }
    inline void on_wheel_scroll(Camera& camera, float dl)
    {
        std::visit([&](auto&& state) { state.on_wheel_scroll(*this, camera, dl); }, _state);
    }
